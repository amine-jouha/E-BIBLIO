...
class BackToOLd extends StatefulWidget {
  const BackToOLd({Key? key}) : super(key: key);

  @override
  State<BackToOLd> createState() => _BackToOLdState();
}

class _BackToOLdState extends State<BackToOLd> {
  ...
  confirm() async {
    ...
    await firebaseFirestore.collection('UserInfo').doc(user!.uid).update({"upgraded" : false}).then((value)
    => Fluttertoast.showToast(msg: 'Back To Normal Account'),
